...
(function (doenerTrainer) {
  ...
  function handleLoad(_event) {
    ...
    imgData = doenerTrainer.crc2.getImageData(
      0,
      0,
      doenerTrainer.crc2.canvas.width,
      doenerTrainer.crc2.canvas.height
    );
  ...
  function start() {
    ...
    if (clickStart === 0) {
      ...
      let workerClass = new doenerTrainer.Workers(
        new doenerTrainer.Vector(20, 100),
        new doenerTrainer.Vector(20, 100),
        false
      );
  ...
  function callWorker() {
    ...
    let workerClass = new doenerTrainer.Workers(
      new doenerTrainer.Vector(randomX, randomY),
      new doenerTrainer.Vector(0, 0),
      false
    );
    workerClass.draw();
  ...
  function displayCapacity() {
    ...
    for (let i = 0; i < displayIDIngredientsJar.length; i++) {
      let ingredientAmountJar = document.getElementById(
        displayIDIngredientsJar[i]
      );
  ...
  function analyseOrder() {
    ...
    if (
      customer.preferences.lettuce === doenerTrainer.orderAnalyser.lettuce &&
      customer.preferences.meat === doenerTrainer.orderAnalyser.meat &&
      customer.preferences.mushrooms ===
        doenerTrainer.orderAnalyser.mushrooms &&
  ...
  function callCustomers() {
    ...
    let customerClass = new doenerTrainer.Customers(
      new doenerTrainer.Vector(600, 0),
      new doenerTrainer.Vector(0, 515)
    );
    customerClass.draw();
  ...
  function reduceVelocity() {
    for (let i = 0; i < moveablesWorker.length; i++) {
      moveablesWorker[i].velocity = new doenerTrainer.Vector(
        Math.random() * 0.5,
        Math.random() * 0.5
      );
    }
  ...
  function moveManagerBack() {
    ...
    moveablesManager[0].velocity = new doenerTrainer.Vector(
      -managerVelocityX,
      -managerVelocityY
    );
